$(document).ready(function() {
    ...
    var table = $('#tablaCancelados').DataTable({
                pageLength : 5,
                ...
                columnDefs: [
                    ...
                    {
                        targets: -1,        
                        defaultContent: "<div class='wrapper text-center'><div class='btn-group'><button class='btnBorrar btn btn-danger' data-toggle='tooltip' title='Eliminar'>"+iconoBorrar+"</button></div></div>"
                    }
                ]	   
            });
    ...
    coleccion.on("child_added", datos => {       
        if(datos.child("pedido").val()=="cancelado"){
    ...
    $("#tablaCancelados").on("click", ".btnBorrar", function() {   
        filaEliminada = $(this);
        Swal.fire({
        ...
        if (result.value) {
            let fila = $('#tablaCancelados').dataTable().fnGetData($(this).closest('tr'));            
            let id = fila[0];
